...
if ([string]::IsNullOrWhiteSpace($providerCwd) -or -not (Test-Path -LiteralPath $providerCwd -PathType Container) -or @(Get-ChildItem -LiteralPath $providerCwd -Force).Count -ne 0) {
    [Console]::Error.WriteLine('CONTRACT_REVIEW_PROVIDER_CONFIGURATION_BLOCKER: Claude requires a fresh empty provider working directory.')
    exit 78
}
...
$metadata=[ordered]@{ cliVersion=($versionOutput -join ' ').Trim(); requestedModel=$model; sessionPersistence=$false; tools=@(); safeMode=$true; settingsSources=@(); skills=$false; plugins=$false; hooks=$false; mcpServers=@(); chrome=$false; promptTransport='stdin'; isolatedWorkingDirectory=$true }
...
try { $result=Write-Output -NoEnumerate $prompt | & $claude @arguments 2>&1 }
