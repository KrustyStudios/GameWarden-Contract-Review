...
$metadata=[ordered]@{ cliVersion=($versionOutput -join ' ').Trim(); requestedModel=$model; reasoningEffort=$effort; ephemeral=$true; ignoreUserConfig=$true; ignoreRules=$true; sandbox='read-only'; disabledFeatures=$disabled; promptTransport='stdin' }
...
$global:LASTEXITCODE=0;$result=Write-Output -NoEnumerate $prompt | & $codex @arguments 2>&1
