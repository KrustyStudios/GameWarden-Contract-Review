...
switch($role){
    ...
    'validator' {
        ...
        if($scenario -in @('STAGE1','ROLE_SCHEMA_STAGE1')){$response.stage1Manifest=@([ordered]@{start=1;end=3;destinations=@('contracts/SAMPLE_CONTRACT.md');name='[sample]';disposition='MOVE';perDestinationNames=@('[sample renamed]');findingIds=@('A1')})}
        if($scenario -eq 'DESTINATION_FRAGMENT_SPLIT'){$response.stage1Manifest=@([ordered]@{start=1;end=3;destinations=@('contracts/SAMPLE_CONTRACT.md','contracts/APP_CONTRACT.md');name='[sample]';disposition='SPLIT';perDestinationNames=@('[sample fragment]','[app fragment]');findingIds=@('A1')})}
        if($scenario -eq 'DUPLICATE_STAGE_DESTINATIONS'){$response.resolutions[0].outcome='ACCEPT_A';$response.resolutions[0].acceptedFindingIds=@('A1');$response.unresolved=@();$response.stage1Manifest=@([ordered]@{start=1;end=3;destinations=@('contracts/SAMPLE_CONTRACT.md','contracts/SAMPLE_CONTRACT.md');name='[sample]';disposition='MOVE';perDestinationNames=@('[sample renamed]');findingIds=@('A1')})}
        if($scenario -eq 'DUPLICATE_STAGE_NAMES'){$response.resolutions[0].outcome='ACCEPT_A';$response.resolutions[0].acceptedFindingIds=@('A1');$response.unresolved=@();$response.stage1Manifest=@([ordered]@{start=1;end=3;destinations=@('contracts/SAMPLE_CONTRACT.md');name='[sample]';disposition='MOVE';perDestinationNames=@('[sample renamed]','[sample renamed]');findingIds=@('A1')})}
        if($scenario -eq 'STAGE_WITH_UNRESOLVED'){$response.stage1Manifest=@([ordered]@{start=1;end=3;destinations=@('contracts/SAMPLE_CONTRACT.md');name='[sample]';disposition='MOVE';perDestinationNames=@('[sample renamed]');findingIds=@('A1')})}
    }
